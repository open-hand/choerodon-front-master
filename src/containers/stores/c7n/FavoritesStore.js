...
class FavoritesStore {
  ...
  @action
  loadData = () => {
    axios.get('/base/v1/bookmarks').then(action(
      (data) => {
        if (!data.failed) {
          this.favorites = data;
          this.sortData();
  ...
  createFavorite(field) {
    this.loading = true;
    return axios.post('/base/v1/bookmarks', JSON.stringify(field))
      .then(action((data) => {
        this.loading = false;
        if (!data.failed) {
  ...
  updateFavorite(value) {
    ...
    return axios.put('/base/v1/bookmarks', JSON.stringify([
      {
        id: this.editId,
        objectVersionNumber,
        sort,
  ...
  @action
  deleteFavorite(id) {
    return axios.delete(`/base/v1/bookmarks/${id}`);
  }
  ...
  @action
  saveSort() {
    return axios.put('/base/v1/bookmarks', JSON.stringify(this.favorites)).then(action(
      (data) => {
        if (!data.failed) {
          // this.favorites = data;
          this.loadData();
